...
const App = () => {
  ...
  const startCropping = (image: ProcessedImage) => {
      ...
      setCrop({ x: 0, y: 0 });
      setCropZoom(1);
      setCropRotation(image.rotation); // Initialize with current rotation
  };
  ...
  const handleCropSave = async () => {
    ...
    try {
        ...
        // Update images list: Replace src with cropped version and reset rotation to 0
        setImages(prev => prev.map(img => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            // Wait for React render and Canvas update
        ...
        alert("Failed to generate PDF. Try downloading individual pages.");
    } finally {
    ...
    <button
      ...
      className={`flex flex-col items-center justify-center p-3 rounded-lg border-2 transition-all ${
        settings.template === id 
          ? 'border-pink-500 bg-slate-700 text-pink-400' 
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col md:flex-row font-sans">
        ...
        <div className="fixed inset-0 z-[60] bg-black flex flex-col animate-in fade-in duration-200">
           <div className="p-4 flex justify-between items-center bg-slate-900/80 backdrop-blur-md z-10 border-b border-slate-800">
                ...
                <div className="flex gap-2">
                    <button 
                        onClick={() => setCroppingImageId(null)}
                    ...
                    <button 
                        ...
                        className="px-4 py-2 bg-pink-600 hover:bg-pink-500 text-white rounded-lg font-medium flex items-center gap-2"
                    >
      ...
      {showReview && !croppingImageId && (
        <div className="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-sm flex flex-col p-6 animate-in fade-in duration-200">
            <div className="flex justify-between items-center mb-6 max-w-7xl mx-auto w-full">
                <div>
                    <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                        <Eye className="w-6 h-6 text-pink-500"/> Review Gallery
                    </h2>
                    <p className="text-slate-400 text-sm">Remove unwanted photos, rotate, or crop them before generating collage.</p>
                </div>
                <button 
            ...
            <div className="flex-1 overflow-y-auto max-w-7xl mx-auto w-full">
                            ...
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3">
                                <div className="flex items-center gap-2">
                                    <button 
                                        ...
                                        className="p-2 bg-slate-200 hover:bg-white text-slate-900 rounded-full transform hover:scale-110 transition-all"
                                        title="Rotate 90°"
                                    >
                                    ...
                                    <button 
                                        ...
                                        className="p-2 bg-blue-500 hover:bg-blue-400 text-white rounded-full transform hover:scale-110 transition-all"
                                        title="Crop Image"
                                    >
                                    ...
                                    <button 
                                        ...
                                        className="p-2 bg-red-500 hover:bg-red-400 rounded-full text-white transform hover:scale-110 transition-all"
                                        title="Remove photo"
                                    >
                ...
                {images.length === 0 && (
                    <div className="h-full flex items-center justify-center text-slate-500">No images found.</div>
                )}
            </div>
            <div className="mt-4 flex justify-center">
                 <button 
                    onClick={() => setShowReview(false)}
                    className="px-8 py-3 bg-pink-600 hover:bg-pink-500 text-white rounded-lg font-medium"
                 >
                    Done Reviewing
                 </button>
            </div>
      ...
      <aside className="w-full md:w-96 bg-slate-800 border-r border-slate-700 p-6 flex flex-col gap-8 overflow-y-auto z-10 shadow-xl md:h-screen md:sticky md:top-0">
        <div>
          <h1 className="text-2xl font-bold bg-gradient-to-r from-pink-500 to-violet-500 bg-clip-text text-transparent mb-2">
            InstaFrame
          </h1>
          <p className="text-sm text-slate-400">Batch Video & Photo to Collage</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <button 
            ...
            className="w-full h-24 border-2 border-dashed border-slate-600 rounded-xl flex flex-col items-center justify-center hover:border-pink-500 hover:bg-slate-700/50 transition-all group"
          >
            <Upload className="w-6 h-6 text-slate-500 group-hover:text-pink-400 mb-2 transition-colors" />
            <span className="text-xs text-slate-400 group-hover:text-slate-200">Select files (Videos & Photos)</span>
          </button>
          <input 
            type="file" 
            ref={fileInputRef} 
          ...
          {images.length > 0 && (
            <button 
                ...
                className="w-full py-2 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors border border-slate-600"
            >
                <Eye className="w-4 h-4" /> Review / Remove Photos ({images.length})
            </button>
          )}
        </div>
          ...
          <div className="space-y-8 animate-in slide-in-from-left-4 fade-in duration-500">
            ...
            <div className="space-y-3">
               <label className="block text-sm font-medium text-slate-300 flex items-center gap-2">
                <LayoutTemplate className="w-4 h-4" /> 2. Choose Template
              </label>
              <div className="grid grid-cols-3 gap-2">
                 {/* Specific 4 Grid */}
                 <TemplateButton id="4-grid" label="4 Box (2x2)" icon={Grid} />
                 
                 {/* Dynamic Grid Control Button */}
                 <button
                    onClick={() => {
                        setSettings({ ...settings, template: 'grid' });
                        setCurrentPage(1);
                    }}
                    className={`flex flex-col items-center justify-center p-3 rounded-lg border-2 transition-all ${
                 ...
                 >
                    ...
                    <span className="text-[10px] uppercase tracking-wider">Dynamic</span>
                 </button>

                 {/* Preset for 3 */}
                 <button
                    onClick={() => {
                        setSettings({ ...settings, template: 'grid', limit: 3, columns: 1 });
                        setCurrentPage(1);
                    }}
                    className={`flex flex-col items-center justify-center p-3 rounded-lg border-2 transition-all ${
                        settings.template === 'grid' && settings.limit === 3
                        ? 'border-pink-500 bg-slate-700 text-pink-400' 
                        : 'border-slate-700 bg-slate-800 text-slate-400 hover:border-slate-500'
                    }`}
                 >
                    <Columns className="w-6 h-6 mb-2" />
                    <span className="text-[10px] uppercase tracking-wider">3 Stack</span>
                 </button>
                   ...
                   <div className="bg-slate-700/30 p-3 rounded-lg border border-slate-700 space-y-3 mt-2">
                       <div>
                           <div className="flex justify-between text-xs text-slate-300 mb-1">
                               ...
                               <span className="font-bold text-pink-400">{settings.limit}</span>
                           </div>
                           <input 
                                type="range" 
                                min="1" 
                                ...
                                onChange={(e) => {
                                    setSettings({...settings, limit: Number(e.target.value)});
                                    setCurrentPage(1);
                                }}
                       ...
                       <div>
                           <div className="flex justify-between text-xs text-slate-300 mb-1">
                               <span>Columns</span>
                               <span className="font-bold text-pink-400">{settings.columns}</span>
                           </div>
                           <input 
                                type="range" 
                                min="1" 
            ...
            <div className="space-y-4 border-t border-slate-700 pt-6">
              <label className="block text-sm font-medium text-slate-300 flex items-center gap-2 justify-between">
                ...
                <label className="flex items-center gap-2 cursor-pointer group">
                    <input 
                        ...
                        className="w-4 h-4 rounded border-slate-600 accent-pink-500"
                    />
                    <span className="text-xs text-slate-400 group-hover:text-white transition-colors">Borderless</span>
                </label>
              </label>
              ...
              <label className="flex items-center justify-between p-3 rounded-lg bg-slate-800 border border-slate-700 cursor-pointer hover:border-pink-500 transition-colors">
                  <div className="flex items-center gap-3">
                      <Camera className={`w-5 h-5 ${settings.polaroid ? 'text-pink-500' : 'text-slate-400'}`} />
                      <div>
                          <span className="text-sm font-medium text-slate-200 block">Polaroid Frame</span>
                          <span className="text-[10px] text-slate-500 block">Add classic photo borders</span>
                      </div>
                  </div>
                  <div className={`w-10 h-6 rounded-full p-1 transition-colors ${settings.polaroid ? 'bg-pink-600' : 'bg-slate-600'}`}>
                      <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${settings.polaroid ? 'translate-x-4' : ''}`} />
                  </div>
                  <input 
                      type="checkbox" 
                      className="hidden"
                      checked={settings.polaroid}
                      onChange={(e) => setSettings({...settings, polaroid: e.target.checked})}
                  />
              </label>
              ...
              <div className={`space-y-4 transition-opacity ${settings.borderless ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
               ...
               <div className="space-y-1 pt-2">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {['#ffffff', '#000000', '#fce7f3', '#e0f2fe', '#fef3c7', '#f1f5f9'].map(color => (
                        <button
                            key={color}
                            onClick={() => setSettings({...settings, backgroundColor: color})}
                            className={`w-6 h-6 rounded-full border-2 ${settings.backgroundColor === color ? 'border-pink-500 scale-110' : 'border-transparent'} transition-all`}
                    ...
                    <input 
                        ...
                        className="w-6 h-6 rounded-full bg-transparent overflow-hidden cursor-pointer p-0 border-0"
                    />
                  </div>
                </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-700 space-y-3">
                 <div className="flex justify-between items-center text-xs text-slate-400 mb-2">
                    <span>Total Pages: <span className="text-white font-bold">{totalPages}</span></span>
                    <span>Total Photos: <span className="text-white font-bold">{images.length}</span></span>
                 </div>

                {isDownloadingAll ? (
                    <button disabled className="w-full py-3 bg-slate-700 text-slate-300 rounded-lg font-medium flex items-center justify-center gap-2 cursor-wait">
                        <Loader2 className="w-4 h-4 animate-spin" /> {downloadStatus || 'Processing...'}
                    </button>
                ) : (
                    <>
                        <button 
                            onClick={() => handleDownloadPage()}
                            className="w-full py-3 bg-slate-700 hover:bg-slate-600 text-white rounded-lg font-medium flex items-center justify-center gap-2 transition-colors"
                        >
                            <Download className="w-4 h-4" /> Download Current Page
                        </button>
                        
                        {totalPages > 1 && (
                            <button 
                                onClick={handleDownloadPDF}
                                className="w-full py-3 bg-pink-600 hover:bg-pink-500 text-white rounded-lg font-medium flex items-center justify-center gap-2 transition-colors shadow-lg shadow-pink-900/20"
                            >
      ...
      <main className="flex-1 bg-slate-950 p-4 md:p-10 flex flex-col items-center justify-start overflow-auto relative h-screen">
        {images.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-slate-500 space-y-4">
             ...
             <p>Upload videos to start generating your collage</p>
          </div>
        ) : (
          <div className={`w-full flex flex-col items-center gap-6 animate-in zoom-in-95 duration-500 pb-20 transition-all ${zoom > 1 ? '' : 'max-w-4xl'}`}>
             ...
             <div className="w-full max-w-[500px] flex justify-between items-center bg-slate-800/50 p-2 rounded-lg backdrop-blur-sm border border-slate-700 sticky top-0 z-30">
                 <button 
                    ...
                    disabled={currentPage === 1 || isDownloadingAll}
                 ...
                 <div className="flex flex-col items-center">
                    <span className="text-sm font-medium text-slate-200">Page {currentPage} of {totalPages}</span>
                    <span className="text-[10px] text-slate-500">
                        {settings.template === 'grid' ? `${settings.limit} items / ${settings.columns} cols` : `${itemsPerPage} items per page`}
                    </span>
                 </div>

                 <button 
                    onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))}
                    disabled={currentPage === totalPages || isDownloadingAll}
                    className="p-2 hover:bg-slate-700 rounded-lg disabled:opacity-30 transition-colors"
                 >
             ...
             <div className="relative z-10">
                 ...
                 {isDownloadingAll && (
                     <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-10 backdrop-blur-[2px]">
                         <div className="bg-slate-800 px-4 py-2 rounded-lg shadow-xl border border-slate-700 flex items-center gap-3">
                            <Loader2 className="w-5 h-5 text-pink-500 animate-spin" />
                            <span className="text-sm font-medium">{downloadStatus || 'Processing...'}</span>
                         </div>
                     </div>
                 )}
             </div>
